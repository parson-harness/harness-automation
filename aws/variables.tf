...
variable "create_istio" {
  description = "Install a minimal shared Istio control plane and ingress gateway."
  type        = bool
  default     = false
}
...
variable "istio_istiod_replica_count" {
  description = "Replica count for istiod."
  type        = number
  default     = 1
}

variable "istio_ingress_gateway_replica_count" {
...
variable "enable_kiali" {
  description = "Install Kiali alongside Istio for basic mesh visibility."
  type        = bool
  default     = false
}
...
variable "kiali_chart_version" {
  description = "Optional Kiali chart version. Leave null to let Helm pick the latest chart from the repository."
  type        = string
  default     = null
}
