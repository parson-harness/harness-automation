...
locals {
  ...
  # ECR repo scope for delegate (prefix or all)
  ecr_repo_suffix = var.ecr_repo_prefix != "" ? "${var.ecr_repo_prefix}*" : "*"
  ecr_repo_arn    = "arn:aws:ecr:${local.region}:${local.acc}:repository/${local.ecr_repo_suffix}"
}
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      { Effect = "Allow", Action = ["ecs:CreateService", "ecs:UpdateService", "ecs:DeleteService", "ecs:RunTask", "ecs:StopTask"], Resource = "*" },
      {
        Effect   = "Allow",
        Action   = ["iam:PassRole"],
        Resource = "arn:aws:iam::${local.acc}:role/harness-ecs-*",
        Condition = {
